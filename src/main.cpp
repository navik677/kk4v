...
int main(int argc, char *argv[]) {
    ...
    // resetting to a disabled cache -- but confirmed on hardware via
    // memory checkpoints that enabling the cache made things WORSE: a
    // decoded image stays cached (and its underlying bitmap memory
    // alive) even after every layer referencing it calls @freeimage,
    // since the cache itself now holds a reference. logo.ks/mirrormoon.ks
    // (one-time intro content, never shown again) went from leaking
    // ~54MiB with no cache to leaking the same ~54MiB WITH a cache that
    // was supposed to help. On a heap this tight, "keep a decoded image
